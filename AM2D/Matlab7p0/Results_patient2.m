...
% ---------------------------------------- %
IRF = [-45 0];
IA = [0 2]; %Maximum allowed disparity in lateral D
alfa_DP = 0.15; % DP regularization weight
% ---------------------------------------- %
% ------------ 2D AM Paerametes ---------- %
% ---------------------------------------- %
midA = 200; % the seed RF-line where the displacement calculation starts from
...
% the displacement of the first 10 and last 10 A-lines is not calculated:
startA = 10; endA = size(Im1,2)-10;
% the displacement of the first 40 and last 40 samples is not calculated:
...
void(void>3) = 3;
void(void<-.0) = -.0;
void = void(startRF:endRF,11:end-10);
figure,  imagesc([0 xRat*(endA-startA)],yRat*[startRF endRF],void);colormap(gray), 
